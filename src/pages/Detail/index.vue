        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(skuAttr, index) in spuSaleAttrList" :key="skuAttr.id">
                <dt class="title">{{ skuAttr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: valueList.isChecked == 1 }"
                  v-for="(valueList, index) in skuAttr.spuSaleAttrValueList"
                  :key="valueList.id"
                  @click="changeActive(valueList, skuAttr.spuSaleAttrValueList)"
                >
...
export default {
  ...
  computed: {
    ...
    //计算给子组件的数据 如果服务器的数据没有回来 skuInfo是个空对象
    skuImageList() {
      return this.skuInfo.skuImageList || [];
    },
    ...mapState({
      googInfo: (state) => state.detail.goodsInfo,
    }),
  },
  components: {
    ImageList,
  ...
  //点击售卖
  methods: {
    changeActive(valueList, arr) {
      //遍历全部售卖属性 设置isChecked 全部为0 清除高亮
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      //点击谁 谁的isChecked 为1 为高亮
    ...
    //加入购物车回调函数
    async addShopcar() {
      //1.发请求 成功和失败进行反馈 传递参数

      //try成功以后干什么 catch失败以后干什么
      try {
